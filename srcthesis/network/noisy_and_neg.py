...
class NoisyAnd(object):
    def __init__(self, sizes):
        ...
        non-input layers"""
        self.biases = [negative_normal((y, 1)) for y in sizes [1:]]
        #print self.biases
        ...
        activation function"""
        self.weights = [negative_normal((y, x))
    ...
    def feedforward(self, a):
        ...
        for b, w in zip(self.biases, self.weights):
            a = cont_and(np.dot(w, np.ones(a.shape)- a) + b)
        return a
    ...
    def update_mini_batch(self, mini_batch, eta):
        ...
        self.weights = self.negative_gradient_Update(self.weights, nabla_w, self.zero_weights, eta, mini_batch)
        self.biases = self.negative_gradient_Update(self.biases, nabla_b, self.zero_biases, eta, mini_batch)
    ...
    # all entries remain negative
    def negative_gradient_Update(self, matrices, gradients, zeros, eta, mini_batch):
        matrices = [w + (eta / len(mini_batch)) * nw
                        for w, nw in zip(matrices, gradients)]
        matrices = [np.minimum(matrix, zero) for matrix, zero in zip(matrices, zeros)]
    ...
    def backprop(self, x, y):
        ...
        for b, w in zip(self.biases, self.weights):
            z = np.dot(w, np.ones(activation.shape)-activation) + b
            zs.append(z)
    ...
    def cost_derivative(self, output_activations, y):
        ...
        return (output_activations - y)/(1-output_activations)
