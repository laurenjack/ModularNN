...
"""Responsible for plotting data sets, so that they may visualized
...
def plot_data_3D(data_set):
    ...
    X1, X2, Y = __reshape_data_3D(data_set)
...
def plot_data_2D(data_set, network):
    #Plot the data
    x, y = __reshape_data_2D(data_set)
    plt.scatter(x, y)
...
def __reshape_data_3D(data_set):
    n = len(data_set)
    X1 = np.zeros(n)
    X2 = np.zeros(n)
    Y = np.zeros(n)
...
def __reshape_data_2D(data_set):
    n = len(data_set)
    X1 = np.zeros(n)
    Y = np.zeros(n)
    for i in xrange(n):
